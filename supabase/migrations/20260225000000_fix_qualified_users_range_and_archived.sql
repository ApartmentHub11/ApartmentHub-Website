-- Migration: Fix qualified_users to use range-based matching and exclude ARCHIVED accounts
--
-- Changes:
-- 1. Replace calculate_qualified_users_for_apartment() with range-based logic:
--    - Computes overall min/max price across ALL price tags for each account
--    - Uses minimum bedroom count from bedroom tags (user wants "at least N")
--    - Apartment qualifies if its price is within [user_min, user_max] AND bedrooms >= user_min_bedrooms
-- 2. Exclude accounts that have 'ARCHIVED' in their tags array
-- 3. Re-backfill all apartments' qualified_users

-- ==========================================
-- 1. Replace the calculation function
-- ==========================================
...
BEGIN
  ...
  WITH account_price_ranges AS (
    ...
    WHERE regexp_match(tag, '(\d+)\s*-\s*(\d+)') IS NOT NULL
      -- Exclude ARCHIVED accounts: skip if ANY tag is 'ARCHIVED' (case-insensitive)
      AND NOT EXISTS (
        SELECT 1 FROM unnest(COALESCE(acc.tags, ARRAY[]::TEXT[])) t
        WHERE UPPER(t) = 'ARCHIVED'
      )
    GROUP BY acc.id
  ),
  account_min_bedrooms AS (
    -- For each account, find the MINIMUM bedroom count across all bedroom tags
    -- (user specifies "minimum bedrooms they need", so we take the smallest)
    SELECT 
      acc.id AS account_id,
  ...
  matching_accounts AS (
    ...
    WHERE 
      ...
      -- If no bedroom tag, we still include (LEFT JOIN, NULL min_bedrooms = no restriction)
...
- Computes overall min/max price from ALL price tags per account
- Apartment price must fall within [overall_min, overall_max]
- Apartment bedrooms must be >= account minimum bedrooms
- Excludes accounts with ARCHIVED tag';

-- ==========================================
-- 2. Backfill: Update qualified_users for all existing apartments
-- ==========================================
